...
## 9. Jenkins Setup Instructions

### 9.1 Required Jenkins Plugins
...
### 9.2 Add SonarCloud Token to Jenkins

In SonarCloud, generate a token.
...
### 9.3 Create Jenkins Pipeline Job
...
In the pipeline configuration:
    ...
    SCM: Git
    Repository URL: https://github.com/YOUR_USERNAME/evat-smartcharge-ai-backend.git
...
### 9.4 Automatic Trigger from Main Branch

To trigger Jenkins automatically when a new commit is pushed to the `main` branch, enable one or both of the following:

    Build Triggers:
    ✓ GitHub hook trigger for GITScm polling
    ✓ Poll SCM: H/2 * * * *

If Jenkins is running locally, GitHub cannot access `localhost` directly. Use ngrok:
...
### 10.1 Create SonarCloud Project

Create a SonarCloud project linked to the GitHub repository.

Project values used in the Jenkinsfile:

    SONAR_PROJECT_KEY = nnpvaan_evat-smartcharge-ai-backend
    SONAR_ORGANIZATION = nnpvaan

### 10.2 Disable Automatic Analysis

Because Jenkins runs CI-based analysis, SonarCloud Automatic Analysis should be disabled.
...
During Jenkins execution, reports are created in the `reports/` folder.

| Report | Description |
|---|---|
| `pytest-report.xml` | JUnit-style Pytest result report |
| `coverage.xml` | Python coverage report used by SonarCloud |
| `bandit-report.json` | Bandit Python security scan report |
| `trivy-image-report.txt` | Trivy Docker image vulnerability report |

The `reports/` folder contains `.gitkeep` so the folder exists in GitHub, but generated reports are ignored by Git.
